        ...
        if item["siteName"] is None or item["siteName"] =="" or \
           ...
           return res.data
            ...
            insertVals.append((siteFile["id"],region,item["siteName"],siteProperty,2,\
                float(item["longitude"]),float(item["latitude"]),clientName,clientProperty,clientAddress,age,grade,number,geojson))
    if len(insertVals)>0:
        #现将批量导入文件信息插入tbl_site_files,并更新tbl_site_files.fileStatus为有效文件
        aiBusModel.updateSiteFileStatus((1,siteFile["id"]))
